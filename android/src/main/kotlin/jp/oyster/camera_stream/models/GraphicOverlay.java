...
public class GraphicOverlay extends View {
     ...
     * Adjusts the y coordinate from the image's coordinate system to the view coordinate system.
     */
    public float translateY(GraphicOverlay overlay, float y) {
        return scale(overlay, y) - overlay.postScaleHeightOffset;
    }

    /**
     * Adjusts the x coordinate from the image's coordinate system to the view coordinate system.
     */
    public float translateX(GraphicOverlay overlay, float x) {
        if (overlay.isImageFlipped) {
            return overlay.getWidth() - (scale(overlay, x) - overlay.postScaleWidthOffset);
        } else {
            return scale(overlay, x) - overlay.postScaleWidthOffset;
        }
    }
     ...
     * Adjusts the supplied value from the image scale to the view scale.
     */
    public float scale(GraphicOverlay overlay, float imagePixel) {
        return imagePixel * overlay.scaleFactor;
    }
    ...
    public void setImageSourceInfo(int imageWidth, int imageHeight, boolean isFlipped) {
        if (imageWidth < 0 || imageHeight < 0) {
            throw new IllegalStateException("image width and height must be positive");
        }
        synchronized (lock) {
